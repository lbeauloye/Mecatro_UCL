...
#define MTC1_REG_INT_ACK      1     // write only (write any value to ack)
...
void Task_MTL1(void)
{
    ...
    MTXLOCK_STDIO();
    printf("Starting MTL1 initialization\n");

    myTouch = MTC_Init(fpga_i2c, fpga_mtc1, LCD_TOUCH_INT_IRQ);
    
    // Enable IRQ for SPI & MTL
    
    OSisrInstall(GPT_SPI_IRQ, (void *) &spi_CallbackInterrupt);
    GICenable(GPT_SPI_IRQ, 128, 1);
    OSisrInstall(GPT_MTC1_IRQ, (void *) &mtc1_CallbackInterrupt);
    GICenable(GPT_MTC1_IRQ, 128, 1);
    
    // Enable interruptmask and edgecapture of PIO core for mtc2 irq
    alt_write_word(fpga_mtc1 + PIOinterruptmask, 0x3);
    alt_write_word(fpga_mtc1 + PIOedgecapture, 0x3);
    
    printf("\nMTL1 initialization completed\n");
    MTXUNLOCK_STDIO();
...
void mtc1_CallbackInterrupt (uint32_t icciar, void *context)
{
    //printf("INFO: IRQ from MTC1\n");
    
    mtc_QueryData(myTouch);
    
    // Clear the interrupt (in i2c core)
    alt_write_word(fpga_i2c + (MTC1_REG_INT_ACK*4), 0x0);
    // Clear the interruptmask of PIO core
    alt_write_word(fpga_mtc1 + PIOinterruptmask, 0x0);
    
    // Enable the interruptmask and edge register of PIO core for new interrupt
    alt_write_word(fpga_mtc1 + PIOinterruptmask, 0x1);
    alt_write_word(fpga_mtc1 + PIOedgecapture, 0x1);
}
