...
void LCD_WR_REG(alt_u32 value);
void LCD_WR_DATA(alt_u32 value);
...
void LCD_WR_REG(alt_u32 value)
{
	alt_write_word(fpga_LT24_RS_addr, 0x00);
	alt_write_word(fpga_LT24_D_addr,  value);
	//usleep(1);
	alt_write_word(fpga_LT24_WR_N_addr, 0x00);
	//usleep(1);
	alt_write_word(fpga_LT24_WR_N_addr, 0x01);
}

void LCD_WR_DATA(alt_u32 value)
{
	alt_write_word(fpga_LT24_RS_addr, 0x01);
	alt_write_word(fpga_LT24_D_addr,  value);
	//usleep(1);
	alt_write_word(fpga_LT24_WR_N_addr, 0x00);
	//usleep(1);
...
{
		alt_u16 x2, y2;
		y2 = x;				// To perform a portrait to landscape transformation
		x2 = (240 - y);
		LCD_SetCursor(x2,y2);
        LCD_WR_REG(0x002C);
        LCD_WR_DATA(color);
...
void LCD_Init()
{
	alt_write_word(fpga_LT24_RESET_N_addr, 0x01);
	Delay_Ms(1);
	alt_write_word(fpga_LT24_RESET_N_addr, 0x00);
